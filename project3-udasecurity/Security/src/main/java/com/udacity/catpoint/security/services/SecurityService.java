...
public class SecurityService {
    ...
    private void catDetected(Boolean cat) {
        isCat = cat;
        boolean isAllSensorInActive = getSensors().stream().allMatch(sensor -> sensor.getActive() == false);
        ArmingStatus armingStatus = getArmingStatus();

        if(cat && armingStatus == ArmingStatus.ARMED_HOME) {
            setAlarmStatus(AlarmStatus.ALARM);
        } else if (!cat && isAllSensorInActive) {
            setAlarmStatus(AlarmStatus.NO_ALARM);
        }
    ...
    public void changeSensorActivationStatus(Sensor sensor, Boolean active) {
        ...
        if (alarmStatus != AlarmStatus.ALARM) {
            ...
            if (!active && armingStatus == ArmingStatus.DISARMED) {
                handleSensorDeactivated();
            }
        }
